...
#
# Usage:
#   ./find_unserved_pop.sh [level] [version] [buffer] [weight_method] [weight_func]
#
...
${PYTHON_CMD} -m research_code.pop_at_risk_river_calculations.find_unserved_pop "${LEVEL}" "${VERSION}" "${BUFFER}" "${WEIGHT_METHOD}" "${WEIGHT_FUNC}" 2>&1 | tee -a "${LOG_DIR}/find_unserved_pop.log"
