...
RESEARCH_CODE_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = RESEARCH_CODE_ROOT.parent
...
def _prepare_config_dict(base_config):
    ...
    cfg["paths"]["final_data_dir"] = "final_data_source"
    cfg["paths"]["extra_points_dir"] = "extra_points"
    cfg["paths"]["dl_dir"] = "DL_results"
    cfg["paths"]["annotations_dir"] = "annotations"
    cfg["paths"]["figures_dir"] = "figures"

    for key, value in list(cfg["paths"].items()):
        if key == "data_dir" or not isinstance(value, str) or "://" in value:
            continue
        if Path(value).is_absolute():
            suffix = Path(value).suffix
            cfg["paths"][key] = f"./external/{key}{suffix}"
...
def base_config():
    config_path = RESEARCH_CODE_ROOT / "config.yaml"
