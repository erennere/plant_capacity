...
from research_code.figures_scripts import piechart_interactive as pif
from research_code.figures_scripts import pop_at_risk_figures as prf
...
def test_pop_at_risk_main_orchestrates_with_patched_io(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(prf.gpd, "read_parquet", lambda path: pop_at_risk.copy())
    ...
    monkeypatch.setattr(prf.duckdb, "sql", lambda q: _DuckResult())
...
def test_piechart_interactive_import_fallback_block_executes():
    module_path = Path(__file__).resolve().parents[3] / "research_code" / "figures_scripts" / "piechart_interactive.py"
...
def test_pop_at_risk_module_main_guard_executes(monkeypatch, tmp_path):
    ...
    import duckdb
    import geopandas
    import research_code.starter as starter_mod
    import research_code.pop_at_risk_river_calculations.find_pop_in_danger_pop as fpip
    ...
    module_path = Path(__file__).resolve().parents[3] / "research_code" / "figures_scripts" / "pop_at_risk_figures.py"
