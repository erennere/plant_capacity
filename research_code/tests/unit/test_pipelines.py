...
def test_run_voronoi_approach_passes_resolved_functions_and_only_round(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("research_code.create_voronoi.orchestrate_voronoi_weights", fake_orchestrate)
...
def test_run_voronoi_approach_writes_tuple_result(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "research_code.create_voronoi.orchestrate_voronoi_weights",
        lambda *args, **kwargs: (region_df, point_df),
    )
    monkeypatch.setattr(
        "research_code.create_voronoi.ensure_output_dir_for_file",
...
def test_prepare_data_uses_final_geometry_and_filters_industrial_sites(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("research_code.create_voronoi.drop_duplicates", lambda gdf, col: gdf)
    monkeypatch.setattr("research_code.create_voronoi.buffer_geometry", lambda geom: geom)
    monkeypatch.setattr("research_code.create_voronoi.intersects_with_country_db", fake_intersects_with_country_db)
    monkeypatch.setattr("research_code.create_voronoi.download_overture_maps", lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("download should not run when overture exists")))
    monkeypatch.setattr("research_code.create_voronoi.intersect_with_polygon_sindex", lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("basin join should not run when HYBAS_ID already exists")))
    monkeypatch.setattr("research_code.create_voronoi.ensure_output_dir_for_file", lambda path: None)
...
def test_prepare_data_csv_mode_downloads_overture_once_and_exports_expanded_outputs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("research_code.create_voronoi.drop_duplicates", lambda gdf, col: gdf)
    monkeypatch.setattr("research_code.create_voronoi.buffer_geometry", lambda geom: geom)
    monkeypatch.setattr("research_code.create_voronoi.download_overture_maps", fake_download)
    monkeypatch.setattr("research_code.create_voronoi.intersects_with_country_db", fake_intersects_with_country_db)
    monkeypatch.setattr("research_code.create_voronoi.intersect_with_polygon_sindex", fake_intersect_with_polygon_sindex)
    monkeypatch.setattr("research_code.create_voronoi.ensure_output_dir_for_file", lambda path: None)
