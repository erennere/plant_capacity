...
from research_code.pop_at_risk_river_calculations import impact_polygons_pop
...
def test_get_runtime_params_uses_defaults_for_invalid_values():
    params = impact_polygons_pop.get_runtime_params(
        {
    ...
    assert params["org_per_pop"] == pytest.approx(60.0)
    assert params["step_m"] == pytest.approx(250.0)
    assert params["impact_radii"] == [300.0, 900.0]
