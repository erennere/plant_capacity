...
#
# Usage:
#   ./grid_generation_and_osm_extract.sh [level] [version] [buffer] [weight_method] [weight_func] [dynamic_buffering] [dynamic_buffer_k]
#
...
${PYTHON_CMD} -m research_code.annotation_scripts.NEW_01_GENERATEGRIDS "${LEVEL}" "${VERSION}" "${BUFFER}" "${WEIGHT_METHOD}" "${WEIGHT_FUNC}" "${DYNAMIC_BUFFERING}" "${DYNAMIC_BUFFER_K}" 2>&1 | tee -a "${LOG_DIR}/grid_generation_osm_extract.log"

log "Running NEW_02_EXTRACTOSMDATAFULL_GEOJSON"
${PYTHON_CMD} -m research_code.annotation_scripts.NEW_02_EXTRACTOSMDATAFULL_GEOJSON "${LEVEL}" "${VERSION}" "${BUFFER}" "${WEIGHT_METHOD}" "${WEIGHT_FUNC}" "${DYNAMIC_BUFFERING}" "${DYNAMIC_BUFFER_K}" 2>&1 | tee -a "${LOG_DIR}/grid_generation_osm_extract.log"
