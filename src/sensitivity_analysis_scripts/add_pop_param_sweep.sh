...
PY
    ...
    if (( num_files == 0 )); then
        log "WARNING: No voronoi files found for level=${level} buffer=${buffer} weight_method=${weight_method} weight_func='${weight_func}' dynamic_buffering=${dynamic_buffering} dynamic_buffer_k=${dynamic_buffer_k} Ã¢â‚¬â€ skipping"
        continue
    fi
