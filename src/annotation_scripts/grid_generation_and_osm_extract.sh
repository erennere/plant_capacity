...
${PYTHON_CMD} -m src.annotation_scripts.NEW_01_GENERATEGRIDS "${LEVEL}" "${VERSION}" "${BUFFER}" "${WEIGHT_METHOD}" "${WEIGHT_FUNC}" "${DYNAMIC_BUFFERING}" "${DYNAMIC_BUFFER_K}" 2>&1 | tee -a "${LOG_DIR}/grid_generation_osm_extract.log"

log "Running NEW_02_EXTRACTOSMDATAFULL_GEOJSON"
${PYTHON_CMD} -m src.annotation_scripts.NEW_02_EXTRACTOSMDATAFULL_GEOJSON "${LEVEL}" "${VERSION}" "${BUFFER}" "${WEIGHT_METHOD}" "${WEIGHT_FUNC}" "${DYNAMIC_BUFFERING}" "${DYNAMIC_BUFFER_K}" 2>&1 | tee -a "${LOG_DIR}/grid_generation_osm_extract.log"
